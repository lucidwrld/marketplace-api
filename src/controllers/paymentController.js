...
const confirmItemDelivery = async (req, res) => {
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            const vendorItemsOnOrder = tx.orderItem.findMany({
            ...
            if(vendorFullyDeliveredEverything){
                const existingPayout = tx.payout.findUnique({
                    where: {orderId: orderItem.id, vendorId: orderItem.vendorId}
                })
                ...
                if(!existingPayout) {
                    ...
                    const payoutAmount = vendorTotal * (1 - (PLATFORM_FEE_PERCENT / 100))
